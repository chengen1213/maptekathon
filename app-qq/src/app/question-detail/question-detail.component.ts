...
// }
 ...
 mySubmit(){
  ...
  const url_Put = this.apiGetQuestion+'/'+id;

  this.http.put(url_Put,{"title":this.formModel.value.title,"description":this.formModel.value.description,"expirationDate":endTime_Formating,
  ...
  downLoadPrivate(datasetId: number) {
    this.questionService.downloadFileSystem(datasetId)
      .subscribe(response => {
  ...
  // downLoadPublic(datasetId: number){
